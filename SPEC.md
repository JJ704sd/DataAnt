...
| `SITE_PROTECTION_CHALLENGE` | 否，整批停止 | 检测到 `sec.douban.com` 的 JS 算力挑战（SHA-512 nonce mining）—— 不自动化绕过，等待 IP 频次窗口过期后重跑 |
| `OUTPUT_LOCKED` | 是 | Excel 被其他程序占用或无法替换 |
| `UNEXPECTED_ERROR` | 是 | 未分类异常，必须保存诊断产物 |

再次运行时，默认重跑 `NETWORK_ERROR`、`OUTPUT_LOCKED` 和 `UNEXPECTED_ERROR`，跳过 `SUCCESS`、`NOT_FOUND`、`REVIEW_REQUIRED`、`PAGE_CHANGED`、`BLOCKED` 和 `SITE_PROTECTION_CHALLENGE`。CLI 提供 `--retry-status` 显式增加需重跑的状态，例如修复定位器后使用 `--retry-status PAGE_CHANGED`。`SITE_PROTECTION_CHALLENGE` 不在 `--retry-status` 白名单内（`--retry-status BLOCKED` 类同样禁止），因为绕过站点风控挑战属于 AGENTS.md 明确禁止的"automate site-protection bypasses"。所有状态都采用 upsert，不新增重复行。
...
- 检测到 `sec.douban.com` 的 JavaScript 算力挑战（`<form name="sec">` + `name="cha"` + `name="sol"` 三个 marker 同时出现）：写入 `SITE_PROTECTION_CHALLENGE`，同样立即关闭 batch。**不**等待浏览器自动算完 nonce，**不**重试，因为那等同于"automate site-protection bypasses"。操作员需要等 IP 频次窗口过期（典型 15-30 分钟）后手工重跑。
