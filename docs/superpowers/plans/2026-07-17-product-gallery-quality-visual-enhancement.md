...
- [ ] **Step 2: 运行 gallery 基线测试**

Run:

~~~powershell
Set-Location -LiteralPath 'D:\DataAnt'
python -m pytest tests/test_product_gallery.py -q
~~~
...
- [ ] **Step 1: 扩展测试导入并添加确定性混合快照 fixture**
...
- [ ] **Step 3: 添加 PARTIAL、未分类和证据侧栏契约测试**
...
- [ ] **Step 3: 在 render_gallery() 中注入质量占位值和格式化时间**
...
- [ ] **Step 4: 运行 Python 首屏测试**

Run:

~~~powershell
Set-Location -LiteralPath 'D:\DataAnt'
python -m pytest tests/test_product_gallery.py -k "quality_summary or timestamps" -q
~~~
...
- [ ] **Step 1: 在 JavaScript 中定义唯一的 UI sentinel 和质量字段映射**
...
- [ ] **Step 3: 让筛选逻辑统一使用 categoryValue()**
...
- [ ] **Step 4: 让 renderSummary() 从当前可见数组重新计算**
...
- [ ] **Step 5: 更新 init() 并删除第二份 summary 依赖**
...
- [ ] **Step 3: 更新摘要 HTML 的 live region 和时间 class**
...
- [ ] **Step 5: 让 fieldRow() 支持 timestamp class 并增强 renderEvidence()**
...
- [ ] **Step 6: 运行 gallery 测试确认视觉层改动不破坏行为**
...
- [ ] **Step 1: 运行 gallery、bundle 和项目配置测试**

Run:

~~~powershell
Set-Location -LiteralPath 'D:\DataAnt'
python -m pytest tests/test_product_gallery.py tests/test_product_output_bundle.py tests/test_verify_products.py tests/test_project_config.py -q
~~~

Expected: 全部通过；bundle verifier 的外部依赖规则仍然禁止 script src、外部字体、fetch、XHR 和 WebSocket。本轮不修改 verifier，因为现有 verifier 已覆盖这些静态契约。

- [ ] **Step 2: 运行全量离线测试和依赖检查**
...
- [ ] **Step 3: 对已有 bundle 执行离线 verifier**
...
if (Test-Path -LiteralPath $bundle) {
    python -m scripts.verify_products --output-dir $bundle
}
~~~
