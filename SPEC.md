...
> **Python 3.11/3.12 + Playwright Python（同步 API）+ openpyxl + pytest**
...
|---|---:|---|---|
| [Microsoft Playwright](https://github.com/microsoft/playwright) | 约 92.8k stars | 现代浏览器测试与确定性自动化 | **采用**；自动等待、定位器、浏览器上下文和 Python API 满足需求 |
| [Selenium](https://github.com/SeleniumHQ/selenium) | 约 34.3k stars | 跨语言、WebDriver 生态、Grid | 可行但不选；本地 Demo 用不到 Grid，Playwright 的等待与上下文更直接 |
...
- 官方 Python API 同时提供同步和异步用法；本项目串行处理，sync 版本更易读。
- Locator 是 Playwright 自动等待与重试能力的核心；点击前会检查元素可见、稳定、可用并能接收事件。
- Browser Context 可保存和复用 cookies/local storage 等登录状态，不需要手工解析原始 Cookie 字符串。
- 支持 trace、截图和有头模式，页面变化时容易复现。
- Chromium、Firefox、WebKit 使用一致 API，Demo 先只安装 Chromium。

参考：[Playwright Locators](https://playwright.dev/python/docs/locators)、[Auto-waiting](https://playwright.dev/python/docs/actionability)、[Authentication](https://playwright.dev/python/docs/auth)。
...
│   ├── browser_session.py      # Playwright 生命周期、trace、登录状态
│   ├── matcher.py              # 确定性匹配规则
...
├── outputs/.gitkeep
├── artifacts/.gitkeep          # 失败截图、trace；不提交真实运行产物
├── .env.example
├── .gitignore
├── pyproject.toml
└── README.md
...
| BrowserSession | 浏览器配置 | page/context | 生命周期、storage state、trace、截图 |
...
- 使用 Playwright 自带 Chromium 和默认 User-Agent，不伪造过期 Chrome UA。
- 一个 batch 复用一个 Browser Context 和一个 Page。
...
定位器初始候选需在实施阶段用 Playwright Inspector 对当前页面确认，并固化到 `douban_movie.py` 与 HTML fixture。不要把搜索结果第一项作为默认答案。
...
2. 用户在打开的浏览器中自行登录；
3. 用户确认后保存 `playwright/.auth/douban.json`；
4. 正常任务通过 `storage_state` 加载。

`playwright/.auth/` 必须加入 `.gitignore`。状态文件可能包含可冒用账号的 Cookie 和 token，不允许通过聊天、邮件或工单传播。
...
- 检测到验证码、访问频率提示或 403/418/429：写入 `BLOCKED`，保存一次脱敏截图和 trace，立即关闭 batch。
...
- 不记录 API Key、Cookie、完整 storage state 或请求头；
- 页面异常保存 `task_id.png` 和 Playwright trace；
- 正常成功不截图，减少敏感数据和磁盘占用；
...
- [ ] 安装 Chromium，运行 Playwright 最小启动脚本；
- [ ] 用 Playwright Inspector 确认当前搜索框、候选和详情字段；
- [ ] 保存脱敏 HTML fixture，并记录页面检查日期；
- [ ] 用 1 个电影名人工确认公开页面是否满足字段需求。

**退出条件**：可见浏览器能完成一次搜索，定位器已通过 Inspector 验证；若站点要求验证或不允许当前访问，停止项目而非继续开发绕过方案。
...
- [ ] 示例 CSV 不含真实敏感数据；
- [ ] `.gitignore` 覆盖 `.env`、`playwright/.auth/`、`outputs/`、`artifacts/`。
...
- [ ] 无 API Key、Cookie 或 storage state 泄漏；
- [ ] 合规门禁已有负责人确认记录；
