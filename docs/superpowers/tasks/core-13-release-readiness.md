...
```text
你是 Core 13 的文档与最终验证代理。工作目录固定为 D:\DataAnt。
只读取本 spec（D:\DataAnt\docs\superpowers/tasks/core-13-release-readiness.md）、README.md、scripts/、tests/、.github/workflows/ 与“Base / prerequisites”列出的工件；不得读取总计划。
...
- 固定仓库：`D:\DataAnt`。先验证 `git rev-parse --show-toplevel` 精确返回此路径。
- 使用已存在的 `D:\DataAnt\.venv\Scripts\python.exe`；禁止创建环境或运行安装。
- Task 1–4 应当已合入 `main`；`ci: add offline core release gate` 提交（位置 69/93）的产物 `.github/workflows/core-offline.yml` 必须存在。
...
把 release readiness 的全部可重复 PowerShell 门禁沉淀到本 spec，让任何持有该文件的人都能在工作目录内一行行复现 12 步验证；同时确认 workbook 校验这道合规硬门禁通过，把 `main` 推进到可以 push 的状态。"操作员知情同意"这件事已经从外部审批表挪到了 CLI 上的 `--live-approved` 开关 + 1–10 行 `--max-queries` 上限，本 spec 不再读取任何独立的 approval evidence。
...
if ($Root -ne 'D:\DataAnt') { throw "Wrong worktree: $Root" }
if (-not (Test-Path -LiteralPath 'D:\DataAnt\.venv\Scripts\python.exe')) { throw 'Missing worktree Python' }
"WORKTREE_OK=$Root"
```

Expected: 打印 `WORKTREE_OK=D:\DataAnt` 后正常结束。
...
Expected: `$Initial` 为空；`.github/workflows/core-offline.yml` 必须存在（这是 `ci: add offline core release gate` 提交的产物，位于 main 历史位置 69/93，不再是 top 3）；打印 `INITIAL_GIT_CLEAN_OK`。若该 workflow 缺失，立即报告 `BLOCKED`，不要清理 dirty 文件。

### 3. pytest

```powershell
Set-Location -LiteralPath 'D:\DataAnt'
& 'D:\DataAnt\.venv\Scripts\python.exe' -m pytest -q
...
```powershell
Set-Location -LiteralPath 'D:\DataAnt'
& 'D:\DataAnt\.venv\Scripts\python.exe' -m pytest `
    --cov=app `
    --cov-report=term-missing `
    --cov-report=json:artifacts/coverage.json -v
if ($LASTEXITCODE -ne 0) { throw 'coverage run failed' }
if (-not (Test-Path -LiteralPath 'D:\DataAnt\artifacts\coverage.json')) { throw 'coverage.json missing' }
"COVERAGE_RUN_OK"
```
...
```powershell
Set-Location -LiteralPath 'D:\DataAnt'
& 'D:\DataAnt\.venv\Scripts\python.exe' -m scripts.verify_core `
...
```powershell
Set-Location -LiteralPath 'D:\DataAnt'
& 'D:\DataAnt\.venv\Scripts\python.exe' -m pip check
...
```powershell
Set-Location -LiteralPath 'D:\DataAnt'
& 'D:\DataAnt\.venv\Scripts\python.exe' -m scripts.browser_smoke
...
```powershell
Set-Location -LiteralPath 'D:\DataAnt'
$Workbook = 'D:\DataAnt\outputs\douban_movies.xlsx'
...
workbook = Path(r'D:\DataAnt\outputs\douban_movies.xlsx')
...
'@ | & 'D:\DataAnt\.venv\Scripts\python.exe' -
...
- [ ] 工作目录与解释器均为 `D:\DataAnt` 绝对路径；`ci: add offline core release gate` 提交的产物 `.github/workflows/core-offline.yml` 存在。
